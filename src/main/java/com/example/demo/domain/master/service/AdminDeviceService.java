...
public class AdminDeviceService {
    ...
    @Transactional
    public ActionResponse deleteDevice(Authentication authentication, String deviceId) {
        ensureMaster(authentication);

        Device device = deviceRepository.findByDeviceId(deviceId)
                .orElseThrow(() -> new CustomException(404, "장치를 찾을 수 없습니다."));

        faceRecognitionResultRepository.deleteByDevice(device);
        deviceCaptureRepository.deleteByDevice(device);
        deviceErrorLogRepository.deleteByDevice(device);
        streamEventRepository.deleteByDevice(device);
        streamDetectionRepository.deleteByDevice(device);
        deviceCommandAckRepository.deleteByDeviceId(device.getDeviceId());
        deviceRepository.delete(device);

        return ActionResponse.success(200, "장치가 삭제되었습니다.");
    }

    @Transactional
    public ActionResponse startCameraTest(Authentication authentication, String deviceId) {
        ensureMaster(authentication);
        Device device = getDeviceForCommand(deviceId);

        try {
            deviceService.sendStartCameraTestCommand(
                    device.getDeviceId(),
                    device.getClassroom(),
                    device.getCaptureIntervalSec()
            );
        } catch (RuntimeException e) {
            throw new CustomException(503, "카메라 테스트 명령 전송에 실패했습니다. MQTT 브로커와 장치 연결 상태를 확인해주세요.");
        }

        return ActionResponse.success(200, "카메라 테스트 시작 명령을 전송했습니다.");
    }

    @Transactional
    public ActionResponse stopCameraTest(Authentication authentication, String deviceId) {
        ensureMaster(authentication);
        Device device = getDeviceForCommand(deviceId);

        try {
            deviceService.sendStopCameraTestCommand(
                    device.getDeviceId(),
                    device.getClassroom()
            );
        } catch (RuntimeException e) {
            throw new CustomException(503, "카메라 테스트 종료 명령 전송에 실패했습니다. MQTT 브로커와 장치 연결 상태를 확인해주세요.");
        }

        return ActionResponse.success(200, "카메라 테스트 종료 명령을 전송했습니다.");
    }
    ...
    private Device getDeviceForCommand(String deviceId) {
        Device device = deviceRepository.findByDeviceId(deviceId)
                .orElseThrow(() -> new CustomException(404, "장치를 찾을 수 없습니다."));

        if (!device.isActive()) {
            throw new CustomException(400, "비활성화된 장치에는 명령을 전송할 수 없습니다.");
        }
