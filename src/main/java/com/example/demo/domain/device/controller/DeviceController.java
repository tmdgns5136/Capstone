...
public class DeviceController {
    ...
    @PostMapping("/camera-test/upload")
    public ResponseEntity<ApiResponse<DeviceUploadData>> uploadCameraTest(Authentication authentication,
                                                                          @RequestParam String deviceId,
                                                                          @RequestParam String capturedAt,
                                                                          @RequestParam(required = false) String uploadRequestId,
                                                                          @RequestPart("imageFile") MultipartFile imageFile) throws IOException {
        ApiResponse<DeviceUploadData> response =
                deviceService.uploadCameraTest(authentication, deviceId, capturedAt, uploadRequestId, imageFile);
        return ResponseEntity.status(response.getStatus()).body(response);
    }
