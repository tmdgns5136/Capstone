...
//            }
    ...
    public ApiResponse<List<CourseStateData>> getCourseState(
        ...
        List<CourseStateData> courseStateData = enrollments.stream()
                ...
                .filter(lecture -> isSameSemester(lecture.getLectureSemester(), semester))
                .map(lecture -> {
                    LectureSession currentSession = lectureSessionRepository
                            .findByLectureAndScheduledAtOrderBySessionStartAsc(lecture, targetDate)
                            .stream()
                    ...
                    Attendance attendance = attendanceRepository
                            .findByLectureSessionAndStudent(currentSession, student)
                            .orElse(null);

                    String status = "TBD";

                    if (attendance != null && attendance.getStudentClassStatus() != null) {
                        status = attendance.getStudentClassStatus().getCode();
                    } else if (attendance != null && attendance.getAttendStatus() == AttendStatus.ATTEND) {
                        status = "SIT";
                    } else if (attendance != null && attendance.getAttendStatus() == AttendStatus.ABSENCE) {
                        status = "AWAY";
                    }
                    ...
                    return CourseStateData.builder()
                            ...
                            .endTime(lecture.getLectureEnd())
                            .room(lecture.getLectureRoom())
                            .sessionStartTime(currentSession.getSessionStart() != null
                                    ? currentSession.getSessionStart().toString()
                                    : null)
                            .status(status)
                            .build();
                })
