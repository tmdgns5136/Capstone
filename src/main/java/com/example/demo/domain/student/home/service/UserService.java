...
//            }
    ...
    // 오늘 수업 목록
    public ApiResponse<List<CourseData>> getTodayCourses(Authentication authentication, String year, String semester, String today){
        String userNum = authentication.getName();
        Student student = studentRepository.findByStudentNum(userNum);
        ...
        List<CourseData> courseData = enrollments.stream()
                ...
                .map(lecture -> CourseData.builder()
                        ...
                        .room(lecture.getLectureRoom()).build()).toList();
    ...
    // 진행중인 수업 조회
    public ApiResponse<List<CourseStateData>> getCourseState(Authentication authentication, String year, String semester, String today){
        String userNum = authentication.getName();
        Student student = studentRepository.findByStudentNum(userNum);
        ...
        List<CourseStateData> courseStateData = enrollments.stream()
                ...
                .filter(lecture -> lecture.getLectureSemester().equals(semester))
                .filter(lecture -> lecture.getLectureDay().equalsIgnoreCase(today))
                .filter(lecture -> lecture.getLectureStart().compareTo(LocalTime.now().toString()) <= 0 && LocalTime.now().toString().compareTo(lecture.getLectureEnd()) <= 0)
                .map(lecture -> CourseStateData.builder()
                        ...
                        .attendancePercent("0%").build()).toList();
