...
public class StreamEventService {
    ...
    public ApiResponse<StreamEventData> saveEvent(
        ...
        // The Pi can upload the recapture immediately. Send the command only
        // after this transaction commits so uploadAttendance can see the event.
        Runnable sendRecapture = () -> {
            try {
                deviceService.sendRecaptureCommand(
                        device.getDeviceId(),
                        lectureSession.getLecture().getLectureId(),
                        device.getClassroom()
                );
            } catch (RuntimeException e) {
                // The next periodic capture can still consume this pending event.
                log.error("[YOLO] RECAPTURE 명령 전송 실패: deviceId={}, eventId={}",
                        device.getDeviceId(), event.getEventId(), e);
            }
        };
