...
export default function ProfessorClassControl() {
  const {
    courses,        // 🌟 백엔드가 이번 학기 강의만 깔끔하게 걸러서 줍니다!
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            {scheduledCourse && selectedCourse?.lectureId === scheduledCourse.lectureId ? (
              ...
              <p className="text-xs text-zinc-400 mb-2">정규 수업 시간이 아닙니다</p>
            )}
            ...
            {selectedCourse ? (
              ...
              <h2 className="text-lg font-bold text-zinc-400">이번 학기에 배정된 강의가 없습니다.</h2>
            )}
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              onChange={(e) => {
                const found = courses.find((c) => String(c.lectureId) === String(e.target.value));
                if (found) setSelectedCourse(found);
              }}
              disabled={isActive || courses.length === 0}
            ...
            >
              {courses.length === 0 ? (
                <option value="">진행 중인 강의 없음</option>
              ) : (
                courses.map((c) => (
      ...
      {/* Class Control Panel (강의가 있을 때만 활성화) */}
      {selectedCourse && (
          ...
          <Card className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                {isActive && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={spring} className="p-4 bg-primary/10 rounded-xl border border-primary/30">
                    ...
                    <div className="text-xs text-primary-dark font-medium">센서 수신 간격: 매 2분 (데모: 5초)</div>
                  </motion.div>
                )}
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="space-y-5">
                <div>
                  ...
                  <div className="h-2 bg-zinc-100 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${attendanceData.total > 0 ? (attendanceData.present / attendanceData.total) * 100 : 0}%` }}
                ...
                {isActive && attendanceData.away > 0 && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={spring} className="p-4 bg-rose-50 rounded-xl border border-rose-100 flex items-center gap-3">
                ...
                {!isActive && (
                  <div className="p-6 bg-zinc-50 rounded-xl border border-dashed border-zinc-200 text-center">
                    <p className="text-sm text-zinc-400 font-medium">모니터링이 시작되면 실시간 데이터가 렌더링됩니다</p>
                  </div>
                )}
              </div>
            </CardContent>
